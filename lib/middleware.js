...
function accessLog(req, res, next) {
  ...
  function afterResponse() {
    ...
    if (!isHealthOrMetricRequest(req)) {
      if (!res.finished) {
        logger.debug(`"${req.method} ${req.path}" NO RESPONSE SENT ${new Date() - time} ms`, meta);
      } else {
        logger.debug(`"${req.method} ${req.path}" ${res.statusCode} ${new Date() - time} ms`, meta);
      }
    }
  }
...
function correlationLog(req, res, next) {
  ...
  if (!correlationId) {
    ...
    if (!isHealthOrMetricRequest(req)) {
      logger.info(`Didn't receive correlationId on path ${req.path}, created: ${correlationId}`, {
